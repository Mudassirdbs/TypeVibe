...
import { DownloadSimple, Info, ShareNetwork, Check } from "@phosphor-icons/react";
...
export function ShareScoreCardButton({ stats }: { stats: ResultStats }) {
  const [copied, setCopied] = useState(false);

  const handleShare = async () => {
    const text = `⌨️ Typevibe Speed Test Results:\n🚀 WPM: ${stats.wpm}\n🎯 Accuracy: ${stats.accuracy}%\n⚡ Raw: ${stats.raw} WPM\n📊 Consistency: ${stats.consistency}%\n\nTry it at: https://typevibe.mudassirasghar.dev`;
    try {
  ...
  return (
    <button className={actionBtnClass} onClick={handleShare} type="button">
      {copied ? (
        <Check aria-hidden className="text-emerald-500" size={15} weight="bold" />
      ) : (
