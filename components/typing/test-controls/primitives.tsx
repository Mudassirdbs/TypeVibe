...
export const groupClass =
  "flex items-center rounded-[16px] bg-black/[0.04] dark:bg-white/[0.04] backdrop-blur-md border border-black/5 dark:border-white/10 p-1 shadow-xs transition-all duration-200 hover:border-black/10 dark:hover:border-white/20";
...
export function Toggle({
  ...
  return (
    <motion.button
      className={cn(
        "flex items-center gap-1 rounded-[10px] px-2.5 py-1.5 font-medium text-[11px] transition-all duration-150",
        active
          ? "text-primary shadow-xs bg-primary/10"
          : "text-muted-foreground/50 hover:text-muted-foreground/80 hover:bg-foreground/[0.03]"
      )}
      onClick={onClick}
      type="button"
      whileTap={{ scale: 0.96 }}
...
export function Selector({
  ...
  return (
    <motion.button
      className={cn(
        ...
        active
          ...
          : "text-muted-foreground/50 hover:text-muted-foreground/80 hover:bg-foreground/[0.02]"
      )}
      onClick={onClick}
      type="button"
      whileTap={{ scale: 0.96 }}
    >
      ...
      {active && (
        <motion.span
          className="absolute inset-0 rounded-[10px] bg-primary/20 dark:bg-primary/15 border border-primary/20 shadow-xs"
          layoutId={layoutId}
          transition={pillEase}
        />
      )}
